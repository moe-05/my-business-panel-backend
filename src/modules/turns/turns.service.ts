...
export class TurnsService {
  ...
  async getTurnsByBranch(branchId: string): Promise<Turn[]> {
    try {
      const res = await this.db.query(queries.turns.getByBranch, [
        branchId,
      ])
  ...
  async updateTurn(turnId: number, data: UpdateTurnDto) {
    try {
      ...
      throw new Error(`Failed to update turn. Internal error. Check the body: ${error}`);
    }
  }

  async deleteTurn(turnId: number) {
    try {
      const res = await this.db.query(queries.turns.deleteTurn, [
        turnId
      ])
